...
export interface UseGithubPrSearchResult {
  ...
  refresh: () => void
  fetchDetails: (number: number) => Promise<GithubPrDetails>
  clearError: () => void
}
...
interface UseGithubPrSearchOptions {
  debounceMs?: number
  enabled?: boolean
}

export function useGithubPrSearch(options: UseGithubPrSearchOptions = {}): UseGithubPrSearchResult {
  ...
  const [results, setResults] = useState<GithubPrSummary[]>([])
  ...
  const executeSearch = useCallback(async (term: string) => {
    ...
    setLoading(true)
    try {
      const payload = await invoke<GithubPrSummary[]>(TauriCommands.GitHubSearchPrs, {
  ...
  const fetchDetails = useCallback(async (number: number) => {
    try {
      const payload = await invoke<GithubPrDetails>(TauriCommands.GitHubGetPrDetails, { number })
      setError(null)
      return {
        ...payload,
