            ...
            <>
                {parts.map((part, index) =>
                    part.toLowerCase() === query.toLowerCase() ? (
                        <span key={index} className="text-accent-blue font-semibold">{part}</span>
                    ) : (
        ...
        <div className="relative" ref={containerRef}>
            <input
                ...
                className={`w-full bg-elevated text-primary rounded px-3 py-2 border ${
                    hasError || (value && !branches.includes(value))
                        ...
                        : 'border-subtle focus:border-accent-blue'
            ...
            {isOpen && filteredBranches.length > 0 && menuGeometry && createPortal(
                <div
                    ...
                    className="bg-elevated border border-subtle rounded-md shadow-lg overflow-auto"
                    style={{
                        position: 'fixed',
                        ...
                        {filteredBranches.map((branch, index) => (
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    ...
                                    {branch === branches[0] && (
                                        <span className="text-xs text-muted ml-2">default</span>
                                    )}
                                </div>
                            </div>
                        ))}
            ...
            {showValidationError && value && !branches.includes(value) && (
                <div className="absolute z-50 w-full mt-1 bg-primary border border-accent-red/50 rounded-md shadow-lg p-2.5">
                    <div className="text-accent-red text-sm flex items-center gap-2">
