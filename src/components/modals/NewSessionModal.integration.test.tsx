...
vi.mock('../shared/SessionConfigurationPanel', () => ({
    SessionConfigurationPanel: ({
        ...
        const resolvedModel = codexModel || codexModelOptions?.[0] || 'gpt-5.2-codex'
        ...
        const nextModel = (() => {
            ...
            return resolvedModel === 'gpt-5.2-codex' ? 'gpt-5.2' : 'gpt-5.2-codex'
        })()
...
describe('NewSessionModal Integration with SessionConfigurationPanel', () => {
    ...
    test('normalizes persisted Codex preferences using discovered models', async () => {
        ...
        await waitFor(() => {
            ...
            expect(payload?.preferences?.model).toBe('gpt-5.2-codex')
    ...
    test('allows selecting Codex model without conflicting CLI args', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByTestId('codex-model-value').textContent).toBe('gpt-5.2-codex')
