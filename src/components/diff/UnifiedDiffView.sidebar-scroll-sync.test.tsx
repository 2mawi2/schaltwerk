...
const changedFiles: ChangedFile[] = [sampleDiff.file, otherDiff.file]

const loadFileDiffMock = vi.fn(async (_session: string | null, file: ChangedFile) => {
  return file.path === 'src/b.ts' ? otherDiff : sampleDiff
})
...
function SidebarHarness({ onSelectionChange }: { onSelectionChange?: (path: string | null) => void }) {
  const [filePath, setFilePath] = useState<string | null>('src/a.ts')
...
describe('UnifiedDiffView sidebar scroll stability', () => {
  ...
  it('does not auto-scroll when selection change comes from scroll syncing', async () => {
    const selections: Array<string | null> = []

    render(<SidebarHarness onSelectionChange={(path) => selections.push(path)} />)
    ...
    fileA!.getBoundingClientRect = () => createRect(0, -220, 800, 400)
    fileB!.getBoundingClientRect = () => createRect(0, 20, 800, 400)

    const initialScrollTop = 400
    ...
    await waitFor(() => expect(selections).toContain('src/b.ts'))

    await waitFor(() => {
      expect(typeof fileChangeHandler).toBe('function')
    })

    const scrollTopBeforeEvent = scrollContainer.scrollTop
    ...
    await act(async () => {
      await fileChangeHandler?.({
        ...
        branch_info: {
          current_branch: 'schaltwerk/demo',
          base_branch: 'main',
          base_commit: 'abc',
          head_commit: 'def',
        },
    ...
    expect(scrollContainer.scrollTop).toBe(scrollTopBeforeEvent)
  })
})
