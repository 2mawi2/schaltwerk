...
export function UnifiedDiffView({
  ...
  const effectiveContinuousScroll = mode === 'history' || isSidebarMode ? true : continuousScroll
  ...
  }, [mode, isSidebarMode, continuousScroll, selectedFile, files, sessionName, persistDiffPreferences])
    ...
    const pumpQueue = () => {
      ...
      while (activeSet.size < MAX_CONCURRENCY && queue.length > 0) {
        ...
        void loadCommitFileDiff({
          ...
          .catch(error => {
            if (!cancelled) {
              logger.warn('[UnifiedDiffView] Failed to prefetch history diff', error)
            }
          })
          .finally(() => {
            if (cancelled) {
    ...
    if (isOpen) {
      ...
      if (!isSidebarMode) {
        ...
        .then(prefs => {
          ...
          inlineSidebarDefaultRef.current = prefs.inline_sidebar_default ?? true
        })
  ...
  const handleFinishReview = useCallback(async () => {
    ...
    try {
      if (selectedKind === 'orchestrator') {
        ...
        logger.warn('[UnifiedDiffView] Finish review had no valid target', { selection })
        return
      }
  ...
  }, [mode, isOpen, showCommentForm, isSearchVisible, onClose, lineSelection, selectedFileIndex, files, scrollToFile, handleFinishReview, setIsSearchVisible, setShowCommentForm, setCommentFormPosition, keyboardShortcutConfig, platform, clearActiveSelection, isSidebarMode])
  ...
  const diffContent = (
    <div className={`flex-1 flex flex-col overflow-hidden min-h-0 w-full relative ${className || ''}`}>
      <DiffViewer
        files={files}
