...
#[tauri::command]
pub async fn schaltwerk_core_get_theme_mode(
) -> Result<schaltwerk::domains::settings::types::ThemeMode, String> {
    let settings_manager = SETTINGS_MANAGER
        .get()
        .cloned()
        .ok_or_else(|| "Settings manager not initialized".to_string())?;
    ...
    let manager = settings_manager.lock().await;
    Ok(manager.get_theme_mode())
}

#[tauri::command]
pub async fn schaltwerk_core_set_theme_mode(
    theme_mode: schaltwerk::domains::settings::types::ThemeMode,
) -> Result<(), String> {
    let settings_manager = SETTINGS_MANAGER
        .get()
        .cloned()
    ...
    manager
        ...
        .map_err(|e| format!("Failed to save theme mode: {e}"))
}
